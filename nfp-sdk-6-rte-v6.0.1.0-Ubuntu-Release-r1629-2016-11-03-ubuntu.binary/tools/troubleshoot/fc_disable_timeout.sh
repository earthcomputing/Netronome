...
#
# File:        fc_disable_timeout.sh
# Description: Disables flow cache timeout (timeout is enabled by default).
...
for fc_timeout in $(${NETRONOME_BIN}/nfp-rtsym --list 2>&1 | grep fc_timeout_settings | awk '{print $1}' 2>/dev/null) ; do
    echo "Disable flow cache timeout: " $fc_timeout
    ${NETRONOME_BIN}/nfp-rtsym $fc_timeout 0x100
    ret=$((ret + $?))
done