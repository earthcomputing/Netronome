 ...
 */
#define NFP_CPP_TRACE_FLG_DROP_READ         (1 <<  0)

/**
 * Don't capture CPP write actions.
 */
#define NFP_CPP_TRACE_FLG_DROP_WRITE        (1 <<  1)

/**
 * Don't capture CPP explicit actions.
 */
#define NFP_CPP_TRACE_FLG_DROP_EXPL         (1 <<  2)
 ...
 */
#define NFP_CPP_TRACE_FLG_IGN_RDVAL         (1 <<  3)
 ...
 */
#define NFP_CPP_TRACE_FLG_POLL              (1 <<  4)
...
{
    return ((toupper(_str_major_minor[0]) - 'A') << 4) | ((_str_major_minor[1] - '0'));
}
 ...
 */
NFP_API
int nfp_cpp_event_as_signal(struct nfp_cpp_event *event, int signum, const struct sigaction *act);
 ...
 */
NFP_API
void nfp_cpp_trace_flags_set(struct nfp_cpp *cpp, unsigned int flags);
 ...
 */
NFP_API
void nfp_cpp_trace_mask_put(struct nfp_cpp *cpp, int byte_width, void *buff, size_t bytes);
