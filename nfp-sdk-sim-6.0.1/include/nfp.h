 ...
 */
NFP_API
struct nfp_chipdata_chip * nfp_device_chip(struct nfp_device *dev);
...
/**
 * Read memory from NFP DRAM.
 *
 * @param dev           NFP device
 * @param buffer        output buffer
 * @param length        number of bytes to read
 * @param offset        offset into NFP DRAM
 *
 * Read contents of NFP DRAM into @a buffer.  @a length and @a offset must
 ...
 */
NFP_API
ssize_t nfp_dram_read(struct nfp_device *dev, void *buffer, unsigned long long length, unsigned long long offset);
 ...
 * @param offset        offset into NFP DRAM
 *
 * Write contents of @a buffer into NFP DRAM.  @a length and @a offset must
 * both be 4-byte aligned.
 *
 ...
 */
NFP_API
ssize_t nfp_dram_write(struct nfp_device *dev, const void *buffer, unsigned long long length, unsigned long long offset);

/**
 * Fill NFP DRAM with given pattern. 
 *
 * @param dev           NFP device
 ...
 *
 * Fill indicated NFP DRAM area with contents of @a buffer.  If
 ...
 */
NFP_API
ssize_t nfp_dram_fill(struct nfp_device *dev, const void *buffer, unsigned long long buf_length, unsigned long long offset, unsigned long long write_length);
 ...
 */
NFP_API
ssize_t nfp_dram64_read(struct nfp_device *dev, void *buffer, unsigned long long length, unsigned long long offset);
 ...
 */
NFP_API
ssize_t nfp_dram64_write(struct nfp_device *dev, const void *buffer, unsigned long long length, unsigned long long offset);
 ...
 */
NFP_API
ssize_t nfp_dram64_fill(struct nfp_device *dev, const void *buffer, unsigned long long buf_length, unsigned long long offset, unsigned long long write_length);
...
/**
 * @ingroup nfp3200-only
 * Read memory from NFP Global Scratch.
 *
 * @param dev           NFP device
 * @param buffer        output buffer
 * @param length        number of bytes to read
 * @param offset        offset into NFP Global Scratch
 *
 * Read contents of NFP Global Scratch into @a buffer.  @a length and
 ...
 */
NFP_API
ssize_t nfp_gs_read(struct nfp_device *dev, void *buffer, unsigned long long length, unsigned long long offset);

/**
 * @ingroup nfp3200-only
 * Write to NFP Global Scratch.
 *
 * @param dev           NFP device
 * @param buffer        input buffer
 * @param length        number of bytes to write
 * @param offset        offset into NFP Global Scratch
 *
 * Write contents of @a buffer into NFP Global Scratch.  @a length and
 * @a offset must both be 4-byte aligned.
 *
 * @a return number of bytes actually written, or -1 on error (and set
 * errno accordingly).
 */
NFP_API
ssize_t nfp_gs_write(struct nfp_device *dev, const void *buffer, unsigned long long length, unsigned long long offset);

/**
 * @ingroup nfp3200-only
 * Fill NFP Global Scratch with given pattern. 
 *
 * @param dev           NFP device
 * @param buffer        input buffer to fill with
 * @param buf_length    number of bytes in input buffer
 ...
 * @param write_length  number of bytes to write
 *
 * Fill indicated NFP Global Scratch area with contents of @a buffer.
 * If @a write_length is larger than @a buf_length the contents of @a buffer
 * will be repeated until the whole area is filled.  @a buf_length,
 * @a offset, and @a write_length must all be 4-byte aligned.
 *
 * @return number of bytes actually written, or -1 on error (and set
 ...
 */
NFP_API
ssize_t nfp_gs_fill(struct nfp_device *dev, const void *buffer, unsigned long long buf_length, unsigned long long offset, unsigned long long write_length);
