...
enum nfp_chipdata_mu_type
{
    ...
    /* EMEM and DDR are viewed as the same on NFP-32xx.
       DDR is invalid for NFP-6xxx */
...
enum nfp_chipdata_mem_type
{
    ...
    /* NFP-6xxx VQDR is not handled as a new memory type.
       Instead, QDR0,1,2,3 are handled as global
       single-island memories for behaviour similar
       to NFP-32xx real and virtual QDR. */
...
struct nfp_chipdata_memory
{
    ...
    /* Base in resoure from which to start allocating symbols for the
     * chip family as a whole. */
    uint64_t alloc_base;
 ...
 */
struct nfp_chipdata_cppat_entry_nfp6000
{
    uint8_t from_iid;
    uint8_t cpp_tgt;
    uint8_t isld0;
...
union nfp_chipdata_cppat_entry
{
    ...
    struct nfp_chipdata_cppat_entry_nfp6000 nfp6000;
};
 ...
 */
NFP_COMMON_API
struct nfp_chipdata *nfp_chipdata_load(
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_comment(
    const struct nfp_chipdata *data);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_family2str(
    int family);
 ...
 */
NFP_COMMON_API
const struct nfp_chipdata_chip *nfp_chipdata_select(
    const struct nfp_chipdata *data,
    const char *chip_id);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_board_chip_id(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
struct nfp_chipdata_chip *nfp_chipdata_chip_new(
    struct nfp_chipdata *data,
    int chip_family, int chip_rev,
    const char *new_id,
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_cpptgt2str(
    const struct nfp_chipdata_chip *chip,
    int cpp_tgt);
 ...
 */
NFP_COMMON_API
const struct nfp_chipdata *nfp_chipdata_from_chip(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
const struct nfp_chipdata_chip *nfp_chipdata_chip_first(
    const struct nfp_chipdata *data);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_chip_file(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_chip_id(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_chip_description(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_chip_revisions(
    const struct nfp_chipdata_chip *chip,
    const int **revisions);
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_island_ids(
    const struct nfp_chipdata_chip *chip,
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_id_first(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_id_last(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_id_next(
    const struct nfp_chipdata_chip *chip,
    int island_id);
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_id_prev(
    const struct nfp_chipdata_chip *chip,
    int island_id);
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_id_type_first(
    const struct nfp_chipdata_chip *chip,
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_id_type_next(
    const struct nfp_chipdata_chip *chip,
    int island_id,
    enum nfp_chipdata_isld_type island_type);
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_id_type_prev(
    const struct nfp_chipdata_chip *chip,
    int island_id,
    enum nfp_chipdata_isld_type island_type);
 ...
 * Example:
 * @code
 * cnt = nfp_chipdata_island_count(selected_nfp6000_chip,
 *          NFP_CHIPDATA_ISLD_TYPE_CRYPTO);
 * @endcode
 *
 */
NFP_COMMON_API
size_t nfp_chipdata_island_count(
    const struct nfp_chipdata_chip *chip,
    enum nfp_chipdata_isld_type island_type);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_island_name(
    const struct nfp_chipdata_chip *chip,
    int island_id);
 ...
 */
NFP_COMMON_API
enum nfp_chipdata_isld_type nfp_chipdata_island_type(
    const struct nfp_chipdata_chip *chip,
    int island_id);
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_island_cpp_targets(
    const struct nfp_chipdata_chip *chip,
 ...
 */
NFP_COMMON_API
int nfp_chipdata_has_cpp_target(
    const struct nfp_chipdata_chip *chip,
    int cpp_tgt_id);
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_has_cpp_target(
    const struct nfp_chipdata_chip *chip,
    int island_id,
    int cpp_tgt_id);
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_serdes_count(
    const struct nfp_chipdata_chip *chip);
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_island_me_count(
    const struct nfp_chipdata_chip *chip,
    int island_id);
 ...
 */
NFP_COMMON_API
enum nfp_chipdata_mu_type nfp_chipdata_island_mu_type(
    const struct nfp_chipdata_chip *chip,
    int island_id);
 ...
 */
NFP_COMMON_API
uint64_t nfp_chipdata_mu_max_size(
    const struct nfp_chipdata_chip *chip,
    int island_id);
 ...
 */
NFP_COMMON_API
uint64_t nfp_chipdata_mem_base(
    const struct nfp_chipdata_chip *chip,
 ...
 */
NFP_COMMON_API
uint64_t nfp_chipdata_mem_alloc_base(
    const struct nfp_chipdata_chip *chip,
    enum nfp_chipdata_mem_type mem_type,
    int island_id);
 ...
 */
NFP_COMMON_API
uint64_t nfp_chipdata_mem_max_size(
    const struct nfp_chipdata_chip *chip,
    enum nfp_chipdata_mem_type mem_type,
    int island_id);
 ...
 */
NFP_COMMON_API
int nfp_chipdata_island_has_mem_type(
    const struct nfp_chipdata_chip *chip,
    enum nfp_chipdata_mem_type mem_type,
    int island_id);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_mem_name(
    const struct nfp_chipdata_chip *chip,
    enum nfp_chipdata_mem_type mem_type);
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_island_mem_types(
    const struct nfp_chipdata_chip *chip,
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_alloc_alias(
    const struct nfp_chipdata_chip *chip,
    const char *name);
 ...
 */
NFP_COMMON_API
const struct nfp_chipdata_alloc_type *
        nfp_chipdata_find_alloc_type(
    const struct nfp_chipdata_chip *chip,
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_alloc_types(
    const struct nfp_chipdata_chip *chip,
 ...
 */
NFP_COMMON_API
size_t nfp_chipdata_cppat_entries(
    const struct nfp_chipdata_chip *chip,
...
const union nfp_chipdata_cppat_entry *nfp_chipdata_cppat_entry_find(
    int chip_family, const union nfp_chipdata_cppat_entry *entries,
    size_t entry_cnt, int from_island, int cpp_tgt
    );
 ...
 */
NFP_COMMON_API
int nfp_chipdata_cppat_encode(
    int chip_family, const union nfp_chipdata_cppat_entry *entries,
    size_t entry_cnt,
    int from_island, int cpp_tgt,
    int to_island, uint64_t *addr
    );
...
/** Get the MU Locality LSB used by the translator for a given CPPAT Table entry.
 *
 ...
 */
NFP_COMMON_API
int nfp_chipdata_cppat_mu_locality_lsb(
    int chip_family, const union nfp_chipdata_cppat_entry *entries,
    size_t entry_cnt,
    int from_island
    );
...
enum nfp_chipdata_mem_class nfp_chipdata_mem_type2class(
    const struct nfp_chipdata_chip *chip, enum nfp_chipdata_mem_type mem_type);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_mem_type2str(
    int chip_family,
    char *s,
    enum nfp_chipdata_mem_type mem_type,
    int island_id, int menum);
 ...
 */
NFP_COMMON_API
enum nfp_chipdata_mem_type nfp_chipdata_idstr2mem_type(
    int chip_family, const char *s, const char **endptr,
    int *island_id, int *menum);
 ...
 */
NFP_COMMON_API
const char *nfp_chipdata_cfg_val(const struct nfp_chipdata_chip *chip,
    const char *key);
