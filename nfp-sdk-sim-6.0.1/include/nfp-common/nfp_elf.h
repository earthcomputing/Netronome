...
#define NFP_ELF_ELF_ST_INFO(ST_BIND, ST_TYPE) \
    ( (((ST_BIND) & 0xF) << 4) + ((ST_TYPE) & 0xF) )

#define NFP_ELF_ELF32_R_SYM(R_INFO) (((R_INFO) >> 8) & 0xFFFFFF)
#define NFP_ELF_ELF32_R_TYPE(R_INFO) ((R_INFO) & 0xFF)
#define NFP_ELF_ELF32_R_INFO(R_SYM, R_TYPE) \
    ( (((R_SYM) & 0xFFFFFF) << 8) | ((uint8_t)(R_TYPE)) )
...
#define NFP_ELF_ELF64_R_INFO(R_SYM, R_TYPE) \
    ( ((uint64_t)((R_SYM) & 0xFFFFFFFF) << 32) | \
    (uint64_t)((R_TYPE) & 0xFFFFFFFF) )
...
#define NFP3200_ELF_GET_SHI_CLUSTER(val) \
    (NFP3200_ELF_GET_SHI_DOMAIN(val))
#define NFP3200_ELF_SET_SHI_CLUSTER(val) \
    (NFP3200_ELF_SET_SHI_DOMAIN(val))
...
#define NFP6000_ELF_GET_SHI_ISLAND(val) \
    (((NFP6000_ELF_GET_SHI_DOMAIN(val)) >> 8) & 0xFF)
#define NFP6000_ELF_SET_SHI_ISLAND(val) \
    (NFP6000_ELF_SET_SHI_DOMAIN(((val) & 0xFF) << 8))
...
#define NFP_ELF_GET_SHI_MEMTYPE(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_ELF_GET_SHI_MEMTYPE(val) : \
    NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_ELF_GET_SHI_MEMTYPE(val) : 0)
#define NFP_ELF_SET_SHI_MEMTYPE(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_ELF_SET_SHI_MEMTYPE(val) : \
    NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_ELF_SET_SHI_MEMTYPE(val) : 0)
#define NFP_ELF_GET_SHI_ISLAND(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_ELF_GET_SHI_ISLAND(val) : \
    NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_ELF_GET_SHI_CLUSTER(val) : 0)
#define NFP_ELF_SET_SHI_ISLAND(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_ELF_SET_SHI_ISLAND(val) : \
    NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_ELF_SET_SHI_CLUSTER(val) : 0)
#define NFP_ELF_GET_SHI_MEID(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_ELF_GET_SHI_MEID(val) : \
    NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_ELF_GET_SHI_MEID(val) : 0)
#define NFP_ELF_SET_SHI_MEID(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_ELF_SET_SHI_MEID(val) : \
    NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_ELF_SET_SHI_MEID(val) : 0)

#define NFP6000_ELF_GET_SHI_INITREG_ISLAND(val) \
    (((val) >> NFP_ELF_SHI_INITREG_ISLAND_LSB) & \
    NFP_ELF_SHI_INITREG_ISLAND_MASK)
#define NFP6000_ELF_SET_SHI_INITREG_ISLAND(val) \
    (((val) & NFP_ELF_SHI_INITREG_ISLAND_MASK) << \
    NFP_ELF_SHI_INITREG_ISLAND_LSB)
#define NFP6000_ELF_GET_SHI_INITREG_CPPTGT(val) \
    (((val) >> NFP_ELF_SHI_INITREG_CPPTGT_LSB) & \
    NFP_ELF_SHI_INITREG_CPPTGT_MASK)
#define NFP6000_ELF_SET_SHI_INITREG_CPPTGT(val) \
    (((val) & NFP_ELF_SHI_INITREG_CPPTGT_MASK) << \
    NFP_ELF_SHI_INITREG_CPPTGT_LSB)
#define NFP6000_ELF_GET_SHI_INITREG_CPPACTRD(val) \
    (((val) >> NFP_ELF_SHI_INITREG_CPPACTRD_LSB) & \
    NFP_ELF_SHI_INITREG_CPPACT_MASK)
#define NFP6000_ELF_SET_SHI_INITREG_CPPACTRD(val) \
    (((val) & NFP_ELF_SHI_INITREG_CPPACT_MASK) << \
    NFP_ELF_SHI_INITREG_CPPACTRD_LSB)
#define NFP6000_ELF_GET_SHI_INITREG_CPPTOKRD(val) \
    (((val) >> NFP_ELF_SHI_INITREG_CPPTOKRD_LSB) & \
    NFP_ELF_SHI_INITREG_CPPTOK_MASK)
#define NFP6000_ELF_SET_SHI_INITREG_CPPTOKRD(val) \
    (((val) & NFP_ELF_SHI_INITREG_CPPTOK_MASK) << \
    NFP_ELF_SHI_INITREG_CPPTOKRD_LSB)
#define NFP6000_ELF_GET_SHI_INITREG_CPPACTWR(val) \
    (((val) >> NFP_ELF_SHI_INITREG_CPPACTWR_LSB) & \
    NFP_ELF_SHI_INITREG_CPPACT_MASK)
#define NFP6000_ELF_SET_SHI_INITREG_CPPACTWR(val) \
    (((val) & NFP_ELF_SHI_INITREG_CPPACT_MASK) << \
    NFP_ELF_SHI_INITREG_CPPACTWR_LSB)
#define NFP6000_ELF_GET_SHI_INITREG_CPPTOKWR(val) \
    (((val) >> NFP_ELF_SHI_INITREG_CPPTOKWR_LSB) & \
    NFP_ELF_SHI_INITREG_CPPTOK_MASK)
#define NFP6000_ELF_SET_SHI_INITREG_CPPTOKWR(val) \
    (((val) & NFP_ELF_SHI_INITREG_CPPTOK_MASK) << \
    NFP_ELF_SHI_INITREG_CPPTOKWR_LSB)
#define NFP6000_ELF_GET_SHI_INITREG_ORDER(val) \
    (((val) >> NFP_ELF_SHI_INITREG_ORDER_LSB) & \
    NFP_ELF_SHI_INITREG_ORDER_MASK)
#define NFP6000_ELF_SET_SHI_INITREG_ORDER(val) \
    (((val) & NFP_ELF_SHI_INITREG_ORDER_MASK) << \
    NFP_ELF_SHI_INITREG_ORDER_LSB)

#define NFP_ELF_GET_SHI_INITREG_ISLAND(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_GET_SHI_INITREG_ISLAND(val) : 0)
#define NFP_ELF_SET_SHI_INITREG_ISLAND(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_SET_SHI_INITREG_ISLAND(val) : 0)
#define NFP_ELF_GET_SHI_INITREG_CPPTGT(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_GET_SHI_INITREG_CPPTGT(val) : 0)
#define NFP_ELF_SET_SHI_INITREG_CPPTGT(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_SET_SHI_INITREG_CPPTGT(val) : 0)
#define NFP_ELF_GET_SHI_INITREG_CPPACTRD(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_GET_SHI_INITREG_CPPACTRD(val) : 0)
#define NFP_ELF_SET_SHI_INITREG_CPPACTRD(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_SET_SHI_INITREG_CPPACTRD(val) : 0)
#define NFP_ELF_GET_SHI_INITREG_CPPTOKRD(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_GET_SHI_INITREG_CPPTOKRD(val) : 0)
#define NFP_ELF_SET_SHI_INITREG_CPPTOKRD(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_SET_SHI_INITREG_CPPTOKRD(val) : 0)
#define NFP_ELF_GET_SHI_INITREG_CPPACTWR(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_GET_SHI_INITREG_CPPACTWR(val) : 0)
#define NFP_ELF_SET_SHI_INITREG_CPPACTWR(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_SET_SHI_INITREG_CPPACTWR(val) : 0)
#define NFP_ELF_GET_SHI_INITREG_CPPTOKWR(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_GET_SHI_INITREG_CPPTOKWR(val) : 0)
#define NFP_ELF_SET_SHI_INITREG_CPPTOKWR(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_SET_SHI_INITREG_CPPTOKWR(val) : 0)
#define NFP_ELF_GET_SHI_INITREG_ORDER(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_GET_SHI_INITREG_ORDER(val) : 0)
#define NFP_ELF_SET_SHI_INITREG_ORDER(chip_family, val) \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? \
    NFP6000_ELF_SET_SHI_INITREG_ORDER(val) : 0)
...
struct nfp_elf_Elf32_Rel
{
    uint32_t    r_offset;
    uint32_t    r_info;
};

struct nfp_elf_Elf32_Nhdr
{
  uint32_t n_namesz;
  uint32_t n_descsz;
  uint32_t n_type;
...
#define NFP_ELF_INITREGENTRY_W0( \
    _not_last_word_, _ctx_, _valtype_, _cpp_offset_hi_ ) \
    (NFP_ELF_INITREGENTRY_WO_NLW(_not_last_word_) | \
     NFP_ELF_INITREGENTRY_WO_CTX(_ctx_) | \
     NFP_ELF_INITREGENTRY_WO_VTP(_valtype_) | \
...
struct nfp_elf
{
    int is_elf64; /* Was original file/buffer ELF64 */

    /* Use ELF64 as working type, with ELF32 section headers and entries
       translated to ELF64 where possible. */
     ...
     * and a MIP _may_ still be found in the first 256KiB of DRAM/EMEM data.
     */
...
NFP_COMMON_API
struct nfp_elf *nfp_elf_from_file(
    const char* filename);

NFP_COMMON_API
struct nfp_elf *nfp_elf_from_buf(
...
NFP_COMMON_API
int nfp_elf_mip_note(
    struct nfp_elf *ectx,
    size_t *mip_shndx,
    uint64_t *mip_sh_offset);

NFP_COMMON_API
struct nfp_elf_Elf64_Ehdr *nfp_elf_filehdr(
    struct nfp_elf *ectx);

NFP_COMMON_API
size_t nfp_elf_sections(
    struct nfp_elf *ectx,
    struct nfp_elf_Elf64_Shdr **shdrs);

NFP_COMMON_API
size_t nfp_elf_symbols(
    struct nfp_elf *ectx,
    struct nfp_elf_Elf64_Sym **syms);

NFP_COMMON_API
int nfp_elf_sec_has_reloc(
    struct nfp_elf *ectx,
    size_t secidx);
...
NFP_COMMON_API
int nfp_elf_sec_is_text(
    struct nfp_elf *ectx,
    size_t secidx);

NFP_COMMON_API
int nfp_elf_sec_is_init_text(
    struct nfp_elf *ectx,
    size_t secidx);

NFP_COMMON_API
int nfp_elf_sym_is_import_var(
    struct nfp_elf *ectx,
    const struct nfp_elf_Elf64_Sym* sym);

NFP_COMMON_API
int nfp_elf_import_var_is_defined(
    struct nfp_elf *ectx,
    const struct nfp_elf_Elf64_Sym* sym);

NFP_COMMON_API
int nfp_elf_import_var_undefine(
    struct nfp_elf *ectx,
    struct nfp_elf_Elf64_Sym* sym);

NFP_COMMON_API
int nfp_elf_import_var_set(
    struct nfp_elf *ectx,
    struct nfp_elf_Elf64_Sym* sym,
    uint64_t val);

NFP_COMMON_API
struct nfp_elf_Elf64_Sym* nfp_elf_find_sym(
    struct nfp_elf *ectx,
    const char* name);
...
NFP_COMMON_API
struct nfp_elf_Elf_meconfig *nfp_elf_meconf(
    struct nfp_elf *ectx,
    int meid);

NFP_COMMON_API
int nfp_elf_me_has_text(
    struct nfp_elf *ectx,
    int meid);

NFP_COMMON_API
int nfp_elf_me_has_init_text(
    struct nfp_elf *ectx,
    int meid);

NFP_COMMON_API
void nfp_elf_ustore_ecc_calculate_and_set_bits(
    struct nfp_elf *ectx,
    uint64_t *word);

NFP_COMMON_API
const char* nfp_elf_sec_name(
    struct nfp_elf *ectx,
    size_t secidx);

NFP_COMMON_API
const char* nfp_elf_sym_name(
    struct nfp_elf *ectx,
    const struct nfp_elf_Elf64_Sym* sym);

NFP_COMMON_API
size_t nfp_elf_sec_initreg_cnt(
    struct nfp_elf *ectx,
    size_t secidx);

NFP_COMMON_API
size_t nfp_elf_sec_initregs(
    struct nfp_elf *ectx,
    size_t secidx,
