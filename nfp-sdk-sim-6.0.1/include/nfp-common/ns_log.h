...
#define NS_LOG_FLAG_STRIPFILE (1 << 1)
/** Print the designator identifier */
#define NS_LOG_FLAG_IDENT (1 << 2)
/** Print the data and time */
#define NS_LOG_FLAG_TIME (1 << 3)
/** Force STDOUT even if a callback is present */
#define NS_LOG_FLAG_FORCESTDOUT (1 << 4)
/** Print log level at the start of the line, e.g [E] for error */
#define NS_LOG_FLAG_PRINTLEVELTAG (1 << 5)
...
do { \
    if ((level) > ns_log_get_module_level(NS_LOG_MODULE)) break; \
    ns_log_module_print(NS_LOG_MODULE, (level), __FILE__, __LINE__, __func__, str, ##__VA_ARGS__); \
   } while (0)

#define NS_LOG_SET_LEVEL(level) do { ns_log_set_module_level(NS_LOG_MODULE, (level)); } while (0)
#define NS_LOG_GET_LEVEL() ns_log_get_module_level(NS_LOG_MODULE)

#else /* NS_LOG_MODULE */
#define NS_LOG(level, str, ...) \
do { if ((level) > ns_log_get_level()) break; \
     ns_log_module_print(-1, (level), __FILE__, __LINE__, __func__, str, ##__VA_ARGS__); \
   } while (0)

#define NS_LOG_SET_LEVEL(level) do { ns_log_set_level((level)); } while (0)
