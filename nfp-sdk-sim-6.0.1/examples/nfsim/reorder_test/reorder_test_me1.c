...
#define _link_sym(x) __link_sym(#x)
 ...
 __import __shared __cls __align(REORDER_RING_SIZE_LW*4) char reorder_ring_mem[REORDER_RING_SIZE_LW*4] ;
...
{
    {
      ...
      unsigned int sequence = RING_OFFSET + (ctx * RING_INTERVAL) ;
      sleep(dly) ;
      local_csr_write(local_csr_mailbox1, sequence + 0x3000 );
      cls_ring_ordered_lock_ptr32( REORDER_RING_NUM, sequence, ctx_swap, &sig ) ;
      local_csr_write(local_csr_mailbox2, sequence );
      sleep(200) ;
      cls_ring_ordered_unlock_ptr32(REORDER_RING_NUM, sequence) ;
