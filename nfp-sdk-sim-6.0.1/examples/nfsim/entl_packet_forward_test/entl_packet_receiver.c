  ...
  for (;;) {
    ...
    if( ret & ENTL_ACTION_SEND ) {
      ...
      if( !(ret & ENTL_ACTION_FORWARD) ) {
        ...
        int sequence = pkt_rxed.nbi_meta.seq & REORDER_RING_SEQ_MASK ;
    ...
    if( ret & ENTL_ACTION_FORWARD ) {
      ...
      sequence = pkt_rxed.nbi_meta.seq & REORDER_RING_SEQ_MASK ;
      // do reordering here
      reorder_lock( ring, sequence ) ;
      ...
      if( s_addr & ECLP_D_MASK ) {
        ...
        if( entry.parent == 0 ) {
          ...
          if( (s_addr & ECLP_H_MASK) && entry.fw_vector & 1 ) {
            // send to host, TBD
          }
          pkt_data.nbi_num = get_nbi( port ) ;
          pkt_data.dir = 1 ; // set direction bit
          pkt_data.sequencer_num = port ; // = port number
          pkt_data.port = pkt_rxed.nbi_meta.port ;
          pkt_data.island_num = pkt_rxed.nbi_meta.pkt_info.isl;
          pkt_data.packet_num = pkt_rxed.nbi_meta.pkt_info.pnum ;
          pkt_data.bls = pkt_rxed.nbi_meta.pkt_info.bls ;
          pkt_data.len = pkt_rxed.nbi_meta.pkt_info.len ;
          pkt_data.seq_num = get_sequence_num( port ) ;
          pkt_data.out_port = port ;
          pkt_data.next_lookup_id = entry.next_lookup[port] ;
          pkt_out = pkt_data ;
        ...
        for( i = 1 ; i < 15 ; i++ ) {
          if( fw_vector & (1 << i) ) {
            ...
            {
              pkt_data.nbi_num = get_nbi( port ) ;
              pkt_data.dir = 0 ; // set direction bit
              pkt_data.sequencer_num = port ; // = port number
              pkt_data.port = pkt_rxed.nbi_meta.port ;
              pkt_data.bls = pkt_rxed.nbi_meta.pkt_info.bls ;
              pkt_data.len = pkt_rxed.nbi_meta.pkt_info.len ;
              ...
              if( sent ) {
                ...
                destination = pkt_clone(
                  ...
                  PKT_CTM_SIZE_256,
                  (__addr40 __mem uint32_t *) pkt_rxed.nbi_meta.pkt_info.muptr,
