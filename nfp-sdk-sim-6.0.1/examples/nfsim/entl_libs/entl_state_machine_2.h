...
#define ENTL_ACTION_TRIGGERD        0x1000
...
// New Definition for Multi State Machine Support
#define ECLP_ETYPE_MASK     (uint64_t)0x000f00000000
#define ECLP_SMSEL_MASK     (uint64_t)0x001000000000
#define ECLP_SMSEL_SHIFT    36
#define GET_ECLP_SMSEL(x)        (((x >> ECLP_SMSEL_SHIFT) & 0x1 )
...
// number of State machines in the system
#define ENTL_MULTI_STATE_MACINE
#define ENTL_NUM_STATE_MACINE 2
#define ENTL_MONITOR_START_COUNT 10
#define ENTL_MONITOR_ERROR_COUNT 4
...
int entl_state_monitor( __lmem entl_state_machine_t *mcn, __lmem entl_state_machine_t *mon ) ;

// error trigger from monitoring state
int entl_state_trigger( __lmem entl_state_machine_t *mcn, uint64_t *addr, uint64_t *alo_data ) ;
...
/**
 * @brief When ENTL_ACTION_TRIGGERD is set, this function should be called to get the destination MAC address for message
 * @param mcn : pointer to the state machine structure
 * @param addr : pointer to MAC destination adress field, returning the value to be sent
 * @return ENTL_ACTION defined above
 */
