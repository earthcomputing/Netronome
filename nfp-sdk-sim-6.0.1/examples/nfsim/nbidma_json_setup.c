...
void show_config(struct dmasetup_data *d)
{
    ...
    for (i = 0; i < 8; i++) {
        ...
        printf("    .drop_enable           = %d\n", d->bp_cfg[i].drop_enable);
        printf("    .ctm_offset            = %d\n", d->bp_cfg[i].ctm_offset);
        printf("    .primary_buffer_list   = %d\n", d->bp_cfg[i].primary_buffer_list);
        printf("    .secondary_buffer_list = %d\n", d->bp_cfg[i].secondary_buffer_list);
        printf("    .ctm_split_length      = %d\n", d->bp_cfg[i].ctm_split_length);
        printf("    .bpe_head              = %d\n", d->bp_cfg[i].bpe_head);
        printf("    .bpe_chain_end         = %d\n", d->bp_cfg[i].bpe_chain_end);
        //cnt = d->bp_cfg[i].bpe_chain_end - d->bp_cfg[i].bpe_head + 1;
        for (j = d->bp_cfg[i].bpe_head; j <= d->bp_cfg[i].bpe_chain_end; j++) {
            printf("    Buffer Pool Entry [%d]:\n", j);
            printf("        .bpe            = %d\n", d->bpechain[j].bpe);
            printf("        .ctm_target     = %d\n", d->bpechain[j].ctm_target);
            printf("        .packet_credits = %d\n", d->bpechain[j].packet_credits);
            printf("        .buffer_credits = %d\n", d->bpechain[j].buffer_credits);
        }
    }
