 ...
 */
static void
dcfl_read_key(__lmem uint32_t *key, __addr40 uint64_t src, uint32_t size)
{
    ...
    while (tocopy > 0) {
        ...
        mem_read32(read_buf, (__addr40 uint64_t *)(src + offset),
                   chunk);
               ...
               __addr40 uint64_t dest)
{
    ...
    mem_write32(write_buf, (__addr40 uint64_t *)dest,
                sizeof(write_buf));
}
