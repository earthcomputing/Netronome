...
__forceinline
int
pif_lookup_store_key(__mem uint64_t *key_store, __lmem uint32_t *parrep)
{
    SIGNAL sig;
           ...
           __mem uint32_t *action_buffer)
{
    ...
    if (d.rule_count > 0) {
        ...
        mem_read32(rule_read, (__addr40 uint64_t *)work_entry.result_addr,
                   sizeof(rule_read));
    ...
    /* read the action data from the table and write it to the action buffer */
    {
         ...
         * expensive multiply
         */
        ruledata = ((__mem uint32_t *)d.table_address) + (rule << d.record_shift);
#ifdef DESPERATE_DEBUG
