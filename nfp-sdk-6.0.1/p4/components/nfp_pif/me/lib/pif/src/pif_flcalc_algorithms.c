    ...
    )
{
    ...
    while (payload_len) {
        ...
        __mem_read8(xfr_rd, (__mem uint8_t *)((uint64_t)payload_base + payload_offset),
    ...
    )
{
    ...
    /* Include payload if required. */
    {
        ...
        __gpr uint32_t ctm_offset;
        __gpr uint32_t mu_offset;

        if (payld_len)
        {
            odd_byte_offset = odd_byte_offset_bmask & PIF_FLCALC_ODD_MEM;
            ctm_offset = (uint32_t)(((uint64_t)pif_pkt_info_global.pkt_buf - (uint64_t)pkt_ctm_ptr40(__ISLAND, pif_pkt_info_global.pkt_num, 0)) & 0xffffffff);
            if (pif_pkt_info_global.split)
            {
                ctm_payld_len = (256 << pif_pkt_info_global.ctm_size) - ctm_offset - payld_offset;
            ...
            payld_base = pif_pkt_info_global.pkt_buf;
            ...
            if (mu_payld_len)
            {
                ...
                payld_base = (__addr40 void *)((uint64_t)pif_pkt_info_global.muptr << 11);
    ...
    )
{
    ...
    /* Determine length of packet in CTM and EMEM. */
    ctm_offset = (uint32_t)(((uint64_t)pif_pkt_info_global.pkt_buf - 
        (uint64_t)pkt_ctm_ptr40(__ISLAND, pif_pkt_info_global.pkt_num, 0)) &
        0xffffffff);
    if (pif_pkt_info_global.split)
    {
        ctm_len = (256 << pif_pkt_info_global.ctm_size) - ctm_offset - payld_offset;
    ...
    mem_addr = (__addr40 void *)((uint64_t)pif_pkt_info_global.pkt_buf +
                                 payld_offset);
    sum32 = ones_sum_add(sum32, ones_sum_mem(mem_addr, ctm_len));
    if (mu_len)
    {
        mu_offset = (256 << pif_pkt_info_global.ctm_size);
        mem_addr = (__addr40 void *)(((uint64_t)pif_pkt_info_global.muptr << 11) +
                                     mu_offset);
