                     ...
                     unsigned int hdrlen)
{
    ...
    __gpr uint32_t SOP = PIF_PKT_SOP(pif_pkt_info_global.pkt_buf, pif_pkt_info_global.pkt_num);
    __gpr uint32_t ctm_len_max = (256 << pif_pkt_info_global.ctm_size) - SOP;
    __mem uint8_t *mu_base_ptr;
    int overshoot = 0, ctm_remnants, tocopy;
    int mu_off = (256 << pif_pkt_info_global.ctm_size);
    ...
    while (tocopy > 0) {
        ...
        for (i = 0; i < _PIF_XFR_LW; i++) {
            write_xfer[i] = lmem[lmoff];
            lmoff += 1;
        }

        __mem_write8(write_xfer,
                     ((__mem uint8_t *)pif_pkt_info_global.pkt_buf) + pktoff,
    ...
    if (overshoot > 0) {
        pktoff = 0;
        mu_base_ptr = (__mem uint8_t *)(((uint64_t)pif_pkt_info_global.muptr) << 11);
                     ...
                     unsigned int varoff_b)
{
    ...
    while (rem_b > 0) {
        if (rem_b > _PIF_XFR_BYTES)
            ...
            xfr_b = rem_b;
        for (i = 0; i < xfr_b; i+=4)
            xfr_wr_data[i/4] = ((__lmem uint32_t *)lmptr)[(lmoff_b + i)/4];
        __mem_write8(xfr_wr_data,
                     ((__mem uint8_t *)pif_pkt_info_global.pkt_buf) + pktoff,
                     xfr_b, _PIF_XFR_BYTES, ctx_swap, &sig);
        rem_b -= xfr_b;
        pktoff += xfr_b;
        lmoff_b += xfr_b;
    }
    ...
    while (rem_b > 0) {
            ...
            xfr_b = rem_b;
        {
            ...
            __cls_read(xfr_rd_data, varptr+off_b, xfr_4b, _PIF_XFR_BYTES, ctx_swap, &sig);
        }
        for (i = 0; i < xfr_b; i+=4)
    ...
    /* Write any remaining bytes of fixed part of header in lmem. */
    rem_b = lmsz - varoff_b;
    while (rem_b > 0) {
        if (rem_b > _PIF_XFR_BYTES)
            xfr_b = _PIF_XFR_BYTES;
            ...
            xfr_b = rem_b;
        for (i = 0; i < xfr_b; i+=4)
            xfr_wr_data[i/4] = *((__lmem uint32_t *)((void *)&lmptr[lmoff_b + i]));
        __mem_write8(xfr_wr_data,
                     ((__mem uint8_t *)pif_pkt_info_global.pkt_buf) + pktoff,
                     xfr_b, _PIF_XFR_BYTES, ctx_swap, &sig);
        rem_b -= xfr_b;
...
void pif_pkt_free_space(unsigned int pktoff, unsigned int hdrlen)
{
    ...
    __addr40 void *mu_base;
    unsigned int mu_len, mu_off;
    int toshift_mu, toshift_mu2ctm, toshift_ctm;
    unsigned int i, chunk;

    if (pif_pkt_info_global.split == 0) {
        mu_len = 0;
        toshift_ctm = pif_pkt_info_global.pkt_len - pktoff - hdrlen;
        toshift_mu2ctm = 0;
        toshift_mu = 0;
    } else {
        __gpr uint32_t SOP = PIF_PKT_SOP(pif_pkt_info_global.pkt_buf, pif_pkt_info_global.pkt_num);
        ...
        toshift_ctm = pif_pkt_info_global.pkt_len - mu_len - pktoff - hdrlen;
        if (mu_len < hdrlen)
            toshift_mu2ctm = mu_len;
        else
            toshift_mu2ctm = hdrlen;
        toshift_mu = pif_pkt_info_global.pkt_len - pktoff - hdrlen - toshift_ctm -
            toshift_mu2ctm;
    }

    /* Update pktinfo */
    pif_pkt_info_global.pkt_len -= hdrlen;
    if (toshift_mu == 0)
        pif_pkt_info_global.split = 0;
    else
        pif_pkt_info_global.split = 1;
    ...
    while (toshift_ctm > 0) {
        ...
        __mem_read8(read_xfer,
            ((__mem uint8_t *)pif_pkt_info_global.pkt_buf) + pktoff + hdrlen,
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);
    ...
    /* Shift MU to CTM */
    mu_base = (__addr40 void *)((uint64_t)pif_pkt_info_global.muptr << 11);
    mu_off = (256 << pif_pkt_info_global.ctm_size) + mu_len - toshift_mu2ctm - toshift_mu;
    ...
    while (toshift_mu2ctm > 0) {
        ...
        __mem_read8(read_xfer,
            (__mem uint8_t *)((uint64_t)mu_base + (uint64_t)mu_off),
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);
    ...
    while (toshift_mu > 0) {
        ...
        __mem_read8(read_xfer,
            (__mem uint8_t *)((uint64_t)((uint64_t)pif_pkt_info_global.muptr << 11) + (uint64_t)mu_off),
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);

        reg_cp(write_xfer, read_xfer, sizeof(read_xfer));

        __mem_write8(write_xfer,
            (__mem uint8_t *)((uint64_t)((uint64_t)pif_pkt_info_global.muptr << 11) + (uint64_t)mu_off - (uint64_t)hdrlen),
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);
...
void pif_pkt_insert_header(unsigned int copylen, unsigned int hdrlen)
{
    ...
    while (tocopy)
    {
        ...
        __mem_read8(read_xfer,
            (__mem uint8_t *)pif_pkt_info_global.pkt_buf + bytes_copied,
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);

        reg_cp(write_xfer, read_xfer, sizeof(read_xfer));

        __mem_write8(write_xfer,
            (__mem uint8_t *)pif_pkt_info_global.pkt_buf  - hdrlen + bytes_copied,
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);
    ...
    pif_pkt_info_global.pkt_len += hdrlen;
    pif_pkt_info_global.pkt_buf = (__mem void *)((uint64_t)pif_pkt_info_global.pkt_buf - hdrlen);
}
...
void pif_pkt_remove_header(unsigned int copylen, unsigned int hdrlen)
{
    ...
    while (tocopy)
    {
        ...
        __mem_read8(read_xfer,
            (__mem uint8_t *)pif_pkt_info_global.pkt_buf + offset - chunk,
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);

        reg_cp(write_xfer, read_xfer, sizeof(read_xfer));

        __mem_write8(write_xfer,
            (__mem uint8_t *)pif_pkt_info_global.pkt_buf + hdrlen + offset - chunk,
            chunk, _PIF_XFR_BYTES, ctx_swap, &sig);
...
void pif_pkt_make_space(unsigned int pktoff, unsigned int hdrlen)
{
    ...
    __xread uint32_t read_xfer[_PIF_XFR_LW];
    __xwrite uint32_t write_xfer[_PIF_XFR_LW];
    __addr40 uint8_t *mu_base_ptr, *ctm_base_ptr;
    ...
    ctm_len_max = (256 << pif_pkt_info_global.ctm_size) - SOP;

    /* How much data will end up in CTM */
    ctm_len_final = pif_pkt_info_global.pkt_len + hdrlen;
    if (ctm_len_final > ctm_len_max)
        ctm_len_final = ctm_len_max;

    /* get the length of data in the ctm */
    ctm_len_init = pif_pkt_info_global.pkt_len;
    ...
    if (pif_pkt_info_global.split == 0)
        ...
        toshift_mu = pif_pkt_info_global.pkt_len - ctm_len_final;

    /* Shift MU "down" */
    mu_off = (256 << pif_pkt_info_global.ctm_size) + pif_pkt_info_global.pkt_len - ctm_len_final;

    mu_base_ptr = (__mem uint8_t *)(((uint64_t)pif_pkt_info_global.muptr) << 11);
    while (toshift_mu > 0) {
        __mem uint8_t *mem_ptr;
    ...
    /* Shift CTM to MU */
    mu_base_ptr = (__mem uint8_t *)(((uint64_t)pif_pkt_info_global.muptr) << 11);
    ctm_base_ptr = (__mem uint8_t *)pif_pkt_info_global.pkt_buf;
    ctm_off = ctm_len_init - toshift_ctm2mu;
    mu_off = (256 << pif_pkt_info_global.ctm_size) + ctm_overshoot;
...
int pif_pkt_dma_packet_down(unsigned int insertlen)
{
    ...
    __gpr uint32_t ctm_buffer_size = 256 << pif_pkt_info_global.ctm_size;
    __gpr uint32_t sop = PIF_PKT_SOP(pif_pkt_info_global.pkt_buf, pif_pkt_info_global.pkt_num);
    ...
    pkt_len_padded = pif_pkt_info_global.pkt_len + sop;         // packet len from start of buffer
    ...
    if (pkt_len_plus_insertlen <= ctm_buffer_size && pif_pkt_info_global.split == 0) {

        /*
        * shifted packet still fits into CTM buffer, copy to MU and back to CTM
        */

        __mem uint8_t *ctm_source_ptr;
        __mem uint8_t *ctm_dest_ptr;
        __mem __addr40 uint8_t *mu_dest_ptr;
        __xread pkt_status_t pkt_status;

        pkt_status_read(__ISLAND, pif_pkt_info_global.pkt_num, &pkt_status);
        ctm_source_ptr = pkt_status.ctm_addr_div256 * 256;
        ctm_dest_ptr = ((__mem uint8_t *)ctm_source_ptr) + insertlen;

        mu_dest_ptr = (__mem __addr40 uint8_t *)((uint64_t)pif_pkt_info_global.muptr << 11);
        mu_dest_ptr = ((__mem uint8_t *)mu_dest_ptr) + insertlen;
        ...
        pif_pkt_info_global.pkt_buf = (__mem void *)((uint64_t)pif_pkt_info_global.pkt_buf + insertlen);

        return 1;

    } else if (pkt_len_plus_insertlen > ctm_buffer_size && pif_pkt_info_global.split == 0)  {
...

    {
        ...
        __mem uint8_t *ctm_offset_0_ptr;
        __xread pkt_status_t pkt_status;
        __mem __addr40 uint8_t *mu_offset_0_ptr = (__mem __addr40 uint8_t *)((uint64_t)pif_pkt_info_global.muptr << 11);


        pkt_status_read(__ISLAND, pif_pkt_info_global.pkt_num, &pkt_status);
        ctm_offset_0_ptr = pkt_status.ctm_addr_div256 * 256;
