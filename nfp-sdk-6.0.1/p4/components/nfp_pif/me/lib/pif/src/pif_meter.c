...
__intrinsic void mem_meter(__xrw uint32_t *mval, __emem void *addr, int colour, int rfc)
{
...
/* update meter timers */
void update_meter_timers(__mem struct pif_meter_data *meter)
{
    ...
    mem_read_atomic(&mdata_rd, (__mem void *)meter, sizeof(mdata_rd));
    ...
    if (ctime_upper - ptime_upper > 1) {
        ...
        mem_write_atomic(&mdata_wr, (__mem void *)meter, sizeof(mdata_wr));
        return;
    }
...
int pif_meter_execute(__mem struct pif_meter_data *meter,
                      int colour_in,
                      int tick)
{
    ...
    mem_meter(&xval32, (__mem void *)offset, colour_in, RFC_2697);
