...
__intrinsic uint32_t rotr(uint32_t x, uint32_t n) {
    uint32_t result;
    uint32_t copy;
   
    __asm {
        alu[copy, n, B, x] 
        __asm dbl_shf[result, copy, x, >>indirect] 
    }
...
static __mem __addr40 __export __shared uint32_t CAMP_HASH_PAD[] = \
...
__intrinsic hash_result_t camp_hash(void *data, size_t length, size_t max_length)
{
    ...
    result.lo += rotr(result.lo, result.hi);
     
    crc = crc_read();
    result.hi += crc;
 
    result.lo ^= rotr(result.hi, result.lo);
    result.lo ^= crc;
