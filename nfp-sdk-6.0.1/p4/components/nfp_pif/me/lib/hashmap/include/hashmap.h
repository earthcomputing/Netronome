...
void __inline __hashmap_lock_init(hashmap_lock_t *lock, __export __mem void *addr) 
{
    lock->state = 1;
    lock->addr = addr;
}
...
#define HASHMAP_DECLARE_PARTITIONED(instance, entries, key_type, value_type, partitions) \
    ...
    __export __mem __inline value_type *__hashmap_##instance##_lookup(hashmap_lock_t *lock, __lmem key_type *key, uint8_t length, __hashmap_op_t op) \
    { \
        __export __mem __hashmap_##instance##_entry_t *data; \
        ...
        __hashmap_lock_init(lock, (__export __mem void *) &(data[hash.hi % (entries / partitions)])); \
retry:  if (__hashmap_lock_shared(lock)) { \
            local_csr_write(local_csr_active_lm_addr_3, (unsigned int) key); \
            mem_read32((void *) &key_xfer, (__export __mem void *) &(((__export __mem __hashmap_##instance##_entry_t *) lock->addr)->key), (length * 4)); \
            ...
            if (op == REMOVE) { \
                ...
                return &(((__export __mem __hashmap_##instance##_entry_t *) lock->addr)->value); \
            } \
        } \
        if (op == ADD) { \
            if (__hashmap_lock_upgrade(lock)) { \
                ...
                mem_write32((void *) &key_xfer, (__export __mem void *) &(((__export __mem __hashmap_##instance##_entry_t *) lock->addr)->key), (length * 4)); \
                return &(((__export __mem __hashmap_##instance##_entry_t *) lock->addr)->value); \
            } \
            goto retry; \
        } \
