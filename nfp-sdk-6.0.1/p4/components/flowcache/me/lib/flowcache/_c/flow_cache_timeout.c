 ...
 * @brief       Functionality to timeout CAM entries depending on configured timeout and
 *              timestamp last used.
 */
...
//     #warning "FC_ENABLE_OVERFLOW not set. Flow cache timeout will have overflow disabled"
...
#ifdef TIMEOUT_TEST
#if (SIMULATION)
    #define TEST_BUCKET_ADDRESS         0x20080002c0
    #define TEST_BUCKET                 0x2c0
    #define TEST_CTL_IP_MONITOR_MSG     0x280047e100
#endif
#endif
...
#ifdef TEST
    local_csr_write(local_csr_mailbox0, (uint32_t)*cam_start_address);
    local_csr_write(local_csr_mailbox1, (uint32_t)*cam_end_address);
    local_csr_write(local_csr_mailbox2, _FC_BUCKETS_ALLOC_SIZE);
    local_csr_write(local_csr_mailbox3, sizeof(fc_cam_bucket_t));
#endif
    ...
    )
{
    ...
    while (entry_idx != 0)
    {
        ...
        if (!lock.exclusive_lock)
        {
            ...
            */
            ...
            if (!lock.exclusive_lock)
            {
                ...
                */
                mem_incr32((__mem void *)&fc_timeout_err_exclusive_lock_fail[ctx()]);
                return lock;
            }
        }
        ...
        */
        ...
        mem_incr32((__mem void *)&fc_timeout_stats_flows_deleted[ctx()]);
...
static void fc_timeout(__gpr uint32_t bucket_mu)
{
    ...
    */
...
#ifdef TEST
    local_csr_write(local_csr_mailbox0, (__mem __addr40 uint32_t *)cam_base_address);
    local_csr_write(local_csr_mailbox1, (uint32_t)cam_end_address);
    local_csr_write(local_csr_mailbox2, (uint32_t)cam_read_address);
#endif
    ...
    if (ctx() == 0x00)
    {
        ...
        * testing purposes
        * Testing on beagle microcode with pcap file
        */
        cam_read_address = (__fc_addr40 fc_cam_bucket_t*)((uint64_t)cam_base_address + FLOW_CACHE_TEST_BUCKET_ADDR);
    }
#endif
            ...
            while (1)
            {
...

                {
                    ...
                    */
                    ...
                    mem_read_atomic(&read_settings.value[0], (__mem void *)&fc_timeout_settings.value[0], 1 << 2);
                    ...
                    */
                    if (flush_entries_flag)
                    {
                        ...
                        */
                        ...
                        mem_bitclr_imm(flush_entries_flag, (__mem void *)&fc_timeout_settings.value[0]);
                     ...
                     */
                    {
                        ...
                        mem_add32_imm(exclusive_lock_fail, (__mem void *)&fc_timeout_err_exclusive_lock_fail[active_context]);
                        mem_add32_imm(shared_lock_fail, (__mem void *)&fc_timeout_err_shared_lock_fail[active_context]);
                        mem_add32_imm(next_ptr_zero, (__mem void *)&fc_timeout_err_next_ptr_zero[active_context]);
                        mem_add32_imm(head_ptr_zero, (__mem void *)&fc_timeout_err_head_ptr_zero[active_context]);
                        mem_incr32((__mem void *)&fc_timeout_stats_loop_counter[active_context]);
...
void main (void)
{
...

    {
        ...
        {
            /*
             * If context 0, signal the next contexts
             */
            if (__nctx_mode() == 4)
            {
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x02));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x04));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x06));
            }
            else
            {
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x01));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x02));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x03));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x04));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x05));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x06));
                local_csr_write(local_csr_same_me_signal,  ((ME_INIT_SIGNAL << 3) | 0x07));
            }
        }
        else
        {
            /*
             * other contexts wait to be signalled
             */
            wait_for_all(&sig);
        }
#endif
    ...
    /*
    * wait for packets to flow and fill flow cache before we test timeout
    */
    sleep(8000);
#endif
