 ...
 *
 * @file        flow_cache.c
 * @brief       Do a lookup using the provided key, if not found then add it to the CAM.
 */
 ...
 */
    ...
    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_first_entry_added = 0;
    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_entry_added = 0;
    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_exact_match = 0;
    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_first_entry_exact_match = 0;
    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_wait_lock = 0;
    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_allocate_error = 0;

    #ifdef FC_JOURNAL

        volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_allocate_error1 = 0;
        volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_allocate_error2 = 0;
        volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_result_added_out_of_bounds = 0;

    #endif

    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_ring_empty = 0;
    volatile __declspec(i28.imem, shared, export) uint32_t fc_stats_overflow = 0;

#else

    volatile __declspec(i28.imem, shared, import) uint32_t fc_stats_first_entry_added;
    volatile __declspec(i28.imem, shared, import) uint32_t fc_stats_entry_added;
    volatile __declspec(i28.imem, shared, import) uint32_t fc_stats_exact_match;
    volatile __declspec(i28.imem, shared, import) uint32_t fc_stats_first_entry_exact_match;
    volatile __declspec(i28.imem, shared, import) uint32_t fc_stats_wait_lock;
    volatile __declspec(i28.imem, shared, import) uint32_t fc_stats_allocate_error;
    ...
    volatile __declspec(i28.imem, shared, import) uint32_t fc_stats_overflow;
...
void fc_global_init()
{
    ...
    uint32_t                i, j;
    __xwrite uint32_t       wr_free_entry_idx[32];
    ...
    */
    ...
    while (i < FC_NUMBER_RING_ENTRIES)
    {
        ...
        __mem_write32(wr_free_entry_idx, free_base_ptr0 + i, 32<<2, 32<<2, sig_done, &sig_1);
        sig_mask = __signals(&sig_1);
#if (FC_LB_EMU > 1)
            __mem_write32(wr_free_entry_idx, free_base_ptr1 + i, 32<<2, 32<<2, sig_done, &sig_2);
            sig_mask |= __signals(&sig_2);
    #if (FC_LB_EMU > 2)
            __mem_write32(wr_free_entry_idx, free_base_ptr2 + i, 32<<2, 32<<2, sig_done, &sig_3);
     ...
     )
{
   ...
   _MEM_RING_CONVERT_RING_TO_HI_ADDRESS(ring_mu);
    ...
    if (mem_ring_pop(ring_number, hi_addr, rd_free_entries, FC_FREE_ENTRIES << 2) == -1)
    {
         ...
         */
        *free_cache_ptr = 0x00;
        mem_incr32((__mem void *)&fc_stats_ring_empty);
        return 0;
    }
 ...
 */
static __gpr uint64_t fc_lookup_payload_addr_with_base_addr(
    __gpr uint32_t              entry_idx,
...
)
{
    ...
    /*
    * Extract the entry pointer, remember it is 3 bytes in length and not 32-bit aligned.
    * The entry idx is the same index in the entry table as well as the payload table.
    */
    {

        /*
        * read key and verify that it is the same
        */
        __mem_read32(rd_key, entry_ptr, (in_key_len + _FC_ENTRY_KEY_wrd) << 2, (in_key_len+ _FC_ENTRY_KEY_wrd) << 2, sig_done, &sig);
        __wait_for_all(&sig);

        /*
        * set the payload MU and payload ptr while waiting for read
        */
        payload_addr = FC_SET_LINKED_PAYLOAD_ADDRESS(payload_base_ptr, entry_idx);

        if (rd_key[_FC_ENTRY_VERSION_wrd] == in_version)
        {
            for (i = 0; i < in_key_len; i++)
            {
                if (in_key_addr[i] != rd_key[i  + _FC_ENTRY_KEY_wrd])
                {
                    payload_addr = _FC_INVALID_PAYLOAD_ADDR;
                    return payload_addr;
                }
            }
    ...
    return _FC_INVALID_PAYLOAD_ADDR;
}
...
)
{
...
#else
        hash_value = camp_hash(in_key_addr, in_key_len);
#endif
    ...
    FC_SELECT_MU_PTR(
        ...
        */
        ...
        bucket_addr = rotl(bucket_addr, __log2(FC_BUCKETS/FC_LB_EMU) + 6);

#ifdef MASTERME
#ifdef FLOW_CACHE_TEST
        {
            uint32_t val = local_csr_read(local_csr_mailbox1);

#if FLOW_CACHE_TEST == 3
            if (val == 0)
            {
                /*
                 * Add CAM entry 1 (1:1)
                 */
                bucket_addr = FLOW_CACHE_TEST_BUCKET_ADDR;
                bucket_mu = 0;
                hash_value.value_0 = FLOW_CACHE_TEST_CAM_ENTRY_1_HASH_VALUE_0;
            }
            else if (val == 1)
            {
                /*
                ...
                bucket_addr = FLOW_CACHE_TEST_BUCKET_ADDR;
                bucket_mu = 0;
                hash_value.value_0 = FLOW_CACHE_TEST_CAM_ENTRY_2_HASH_VALUE_0;
            }
            else if (val == 2)
            {
                /*
                ...
                bucket_addr = FLOW_CACHE_TEST_BUCKET_ADDR;
                bucket_mu = 0;
                hash_value.value_0 = FLOW_CACHE_TEST_CAM_ENTRY_3_HASH_VALUE_0;
            }
            else if (val == 3)
            {
                /*
                ...
                bucket_addr = FLOW_CACHE_TEST_BUCKET_ADDR;
                bucket_mu = 0;
                hash_value.value_0 = FLOW_CACHE_TEST_CAM_ENTRY_4_HASH_VALUE_0;
            }
            else if (val == 4)
            {
                 /*
                 * Add CAM entry 2 .. first linked entry
                 */
                bucket_addr = FLOW_CACHE_TEST_BUCKET_ADDR;
                bucket_mu = 0;
                hash_value.value_0 = FLOW_CACHE_TEST_CAM_ENTRY_2_HASH_VALUE_0;
            ...
            {
                 /*
                 * Add CAM entry 3 .. first linked entry
                 */
                bucket_addr = FLOW_CACHE_TEST_BUCKET_ADDR;
                bucket_mu = 0;
                hash_value.value_0 = FLOW_CACHE_TEST_CAM_ENTRY_3_HASH_VALUE_0;
            ...
            {
                 /*
                 * Add CAM entry 4 .. first linked entry
                 */
                bucket_addr = FLOW_CACHE_TEST_BUCKET_ADDR;
                bucket_mu = 0;
                hash_value.value_0 = FLOW_CACHE_TEST_CAM_ENTRY_4_HASH_VALUE_0;
            ...
            val++;
            local_csr_write(local_csr_mailbox1,  val);
        }
#endif
#endif
#endif
        ...
        if (!lock_slot.hit)
        {
            ...
            */
            __mem_cam128_lookup24_add_inc(
                &xrw_cam_lookup,
                (__mem __align16 void *)(bucket_ptr),
                sig_done,
                &cam_sig_pair
            );
                ...
                */
                        ...
                        */
                        ...
                        if (result_code != INTERNAL_LOOKUP_KEY_MISMATCH)
                        {
                            ...
                            */
                            ...
                            mem_incr32((__mem void *)&fc_stats_first_entry_exact_match);
                ...
                else if ((xrw_cam_lookup.search.value & 0x8080) == 0x8000)
                {
                    ...
                    */
                    ...
                    fc_lock_cache_base[lock_slot_idx].busy_bit = 0;
                    flag = 1;
                    mem_incr32((__mem void *)&fc_stats_wait_lock);
                } // if exclusive lock
                ...
                else if (xrw_cam_lookup.search.value == 0xff)
                {
                    ...
                    */
                    ...
                    mem_incr32((__mem void *)&fc_stats_overflow);
                ...
                */
                ...
                if (xrw_cam_lookup.search.value == 0xff)
                {
                    ...
                    */
                     ...
                     * Overflow list has to be searched and if entry not in overflow list; it must be added
                     */
            ...
            */
            ...
            if ((lookup_result.search.value & 0xff) == 0x80)
            {
                ...
                */
                ...
                mem_incr32((__mem void *)&fc_stats_first_entry_added);
                return result;
            }
                ...
                */
                ...
                else
                {
                    ...
                    */
                    ...
                    result.payload_address = fc_lookup_payload_addr_with_base_addr(
                                    entry_idx,
                                    entry_ptr,
                                    in_key_len,
                                    in_key_addr,
                    ...
                    */
                    if (result.payload_address != _FC_INVALID_PAYLOAD_ADDR)
                    {
...
#endif
                        mem_incr32((__mem void *)&fc_stats_exact_match);
                        result.result_code = FC_LOOKUP_EXACT_MATCH;
            ...
            if ((lookup_result.search.value & 0x80) == 0x80)
            {
                ...
                */
                ...
                if (entry_idx == 0)
                {
                    ...
                    */
                    ...
                    mem_incr32((__mem void *)&fc_stats_allocate_error);
#ifdef FC_JOURNAL
                    mem_incr32((__mem void *)&fc_stats_allocate_error1);
#endif
                ...
                */
                ...
                mem_incr32((__mem void *)&fc_stats_entry_added);
                result.result_code = FC_LOOKUP_ENTRY_ADDED;
                return result;
            } // else
                    ...
                    */
                    ...
                    if (next_idx == 0)
                    {
                        ...
                        */
                        mem_incr32((__mem void *)&fc_stats_allocate_error);
#ifdef FC_JOURNAL
                        mem_incr32((__mem void *)&fc_stats_allocate_error2);
#endif
                        result.result_code = FC_LOOKUP_ALLOCATE_ERROR;
                        return result;
                    }
