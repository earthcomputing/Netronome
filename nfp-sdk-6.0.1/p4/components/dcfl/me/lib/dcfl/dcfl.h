...
#define DCFL_TESTRES_RING_SIZE_LW   2048
...
#define DCFL_MAX_TEST_REQUESTS      DCFL_KEYQ_ENTRIES
...
struct dcfl_an_workq_entry {
    union {
        __packed struct {
            uint32_t field_idx;
            __addr40 uint8_t *key_ptr;
            __addr40 uint8_t *cfg_ptr;
            __addr40 struct dcfl_label_set *labelset_ptr;
            __addr40 uint32_t *nlabels_ptr;
            __addr40 uint32_t *thread_info_ptr;
 ...
 *
 */
uint32_t dcfl_classify_pipeline(__addr40 uint8_t *key,
                       uint32_t table,
