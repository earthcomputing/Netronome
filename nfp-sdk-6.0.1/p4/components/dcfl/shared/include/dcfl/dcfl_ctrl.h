...
/* Number of long words per block in AN */
...
struct dcfl_me_field_config {
    union {
        struct __attribute__((__packed__)) {
            ...
            uint32_t hdr_offset     :6;      /* The word offset */
...
struct dcfl_me_an_block {
    struct {
        ...
        uint32_t label      :16;
    } entries[DCFL_AN_BLOCK_NUM_ENTRIES];
    uint32_t linked_block_ptr;
};

/* Final stage of AN has different struct, but must be the same size */
#if defined(__GNUC__) &&  (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
struct __attribute__((__packed__)) dcfl_me_an_final_block {
#elif defined(__MSC_VER) &&  (__BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__)
struct __PACKED__ dcfl_me_an_final_block {
#elif defined(__NFP_LANG_MICROC)
...
#define DCFL_FIELD_CFG_HDR_OFFSET_bf        0, 8, 3
#define DCFL_FIELD_CFG_HDR_BITLEN_bf        0,15, 9
#define DCFL_FIELD_CFG_HDR_SHF_bf           0,20,16
