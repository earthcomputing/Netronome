...
static nfp_inline int _nfp6000_decode_basic(uint64_t addr,
    int *dest_island,
    int cpp_tgt,
    int mode,
    int addr40,
 ...
 */
static nfp_inline int _nfp6000_encode_basic(uint64_t *addr,
    int dest_island,
    int cpp_tgt,
    int mode,
    int addr40,
    ...
    int isld0)
{
    ...
    case NFP6000_CPPTGT_MU:
        return NFP_ERRNO(EINVAL); // This function doesn't handle MU
    case NFP6000_CPPTGT_CTXPB:
        return NFP_ERRNO(EINVAL); // Also not CTXPB
    default:
        break;
    }
    ...
    case 0:
        if ((cpp_tgt == NFP6000_CPPTGT_VQDR) && !addr40)
        {
            ...
            if ((dest_island != -1) && (dest_island != v))
                return NFP_ERRNO(EINVAL); // The current address won't go where expected.
    ...
    int isld0)
{
    ...
    case 0:
        ...
        // to get to sram.
        //if ((cpp_tgt == NFP6000_CPPTGT_VQDR) && !addr40)
        //    return NFP_ERRNO(EINVAL); // Full Island ID and channel bits overlap
...
static nfp_inline int _nfp6000_cppat_mu_locality_lsb(int mode,
    int addr40)
{
    switch (mode)
    {
...
static nfp_inline int _nfp6000_encode_mu(uint64_t *addr,
    int dest_island,
    int mode,
    int addr40,
    int isld1,
    int isld0)
{
    ...
    case 3:
        ...
        if ((dest_island > 0) &&
            ...
            *addr |= ((uint64_t)_NIC_NFP6000_MU_LOCALITY_DIRECT) << locality_lsb;
            da = 1;
        }
...
static nfp_inline int _nfp6000_decode_mu(uint64_t addr,
    int *dest_island,
    int mode,
    int addr40,
    int isld1,
...
static nfp_inline int _nfp6000_cppat_addr_encode(uint64_t *addr,
    int dest_island,
    int cpp_tgt,
    int mode,
    int addr40,
    ...
    int isld0)
{
    ...
    case NFP6000_CPPTGT_CTXPB:
        ...
        *addr |= (((uint64_t)dest_island) << 24) & _nic_mask64(29, 24, 0);
        return 0;
    default:
        break;
    }
...
static nfp_inline int _nfp6000_cppat_addr_decode(uint64_t addr,
    int *dest_island,
    int cpp_tgt,
    int mode,
    int addr40,
    ...
    int isld0)
{
    ...
    case NFP6000_CPPTGT_MU:
        return _nfp6000_decode_mu(addr, dest_island, mode,
                                  addr40, isld1, isld0);
...
static nfp_inline int _nfp6000_cppat_addr_iid_clear(uint64_t *addr,
    int cpp_tgt,
    int mode,
    int addr40)
{
    ...
    case NFP6000_CPPTGT_MU:
        locality_lsb = _nfp6000_cppat_mu_locality_lsb(mode, addr40);
        da = (((*addr >> locality_lsb) & 3) == _NIC_NFP6000_MU_LOCALITY_DIRECT);
        switch (mode)
        {
        case 0:
            iid_lsb = (addr40) ? 32 : 24;
