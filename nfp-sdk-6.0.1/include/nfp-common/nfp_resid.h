 ...
 *
 * @file          nfp_resid.h
 * @brief         Resource identification macros and functions.
 *
 * An ME ID or Island ID of -1 indicates an invalid ID.
 * The general usage of this API is to build numeric resource IDs from given
 * parameters (like a Microengine ID to identify a specific microengine in a
 * specific island for NFP-6xxx and also convert such IDs to and from string
 * representations (like "i32.me3" to identify ME 3 in island 32 for NFP-6xxx)
 *
 */
 ...
 */
#define NFP_CHIP_FAMILY_NFP3200 0x3200
...
/** CPP Targets for each chip family
 */
#define NFP3200_CPPTGT_MSF0   1
#define NFP3200_CPPTGT_QDR    2
#define NFP3200_CPPTGT_MSF1   3
#define NFP3200_CPPTGT_HASH   4
#define NFP3200_CPPTGT_MU     7
#define NFP3200_CPPTGT_GS     8
#define NFP3200_CPPTGT_PCIE   9
#define NFP3200_CPPTGT_ARM    10
#define NFP3200_CPPTGT_CRYPTO 12
#define NFP3200_CPPTGT_CAP    13
#define NFP3200_CPPTGT_CT     14
#define NFP3200_CPPTGT_CLS    15
 ...
 */
static nfp_inline uint8_t NFP_CPP_ID_TARGET_of(uint32_t id)
{
 ...
 */
static nfp_inline uint8_t NFP_CPP_ID_TOKEN_of(uint32_t id)
{
	return (id >> 16) & 0xff;
}
 ...
 */
static nfp_inline uint8_t NFP_CPP_ID_ACTION_of(uint32_t id)
{
	return (id >> 8) & 0xff;
}
 ...
 * @return      NFP CPP action
 */
static nfp_inline uint8_t NFP_CPP_ID_ISLAND_of(uint32_t id)
{
        return (id) & 0xff;
}
...
/** Check if @p chip_family is an NFP-32xx chip.
 * @param chip_family One of:
 * NFP_CHIP_FAMILY_NFP3200
 * NFP_CHIP_FAMILY_NFP6000
 */
#define NFP_FAMILY_IS_NFP3200(chip_family) \
    ((int)(chip_family) == (int)NFP_CHIP_FAMILY_NFP3200)
...
/** Make microengine ID for NFP-32xx.
 * @param cluster_num     Cluster number
 * @param menum           ME number, 0 based, within cluster.
 *
 * NOTE: cluster_num should really be unsigned - MSC compiler throws
 * error (not warning) if a clause is always true
 * i.e. cluster_num >= 0 if cluster_num is type unsigned int
 * hence the cast of the cluster_num to an int in that particular clause
 */
#define NFP3200_MEID(cluster_num, menum) \
    ((((int)(cluster_num) >= 0) && ((int)(cluster_num) < 5) && \
    (((int)(menum) & 0x7) == (int)(menum))) ? \
    (int)(((cluster_num) << 4) | (menum) | (0x8)) : -1)


/** Convert a linear NFP-32xx ME number to an MEID.
 *
 * This macro is only used to support linear ME numbers in NFP-32xx
 * NFFW data structures.
 *
 * @param melinnum     Linear ME number (0 to 39)
 */
#define NFP3200_MELIN2MEID(melinnum) \
    NFP3200_MEID(((melinnum) >> 3), ((melinnum) & 0x7))

/** Convert an NFP-32xx MEID to a linear NFP-32xx ME number.
 *
 * This macro is only used to support linear ME numbers in NFP-32xx
 * NFFW data structures.
 *
 * @param meid  MEID
 */
#define NFP3200_MEID2MELIN(meid) \
    ((NFP3200_MEID_CLUSTER_of(meid) * 8) + NFP3200_MEID_MENUM_of(meid))

/** Do a general sanity check on the ME ID.
 * The check is on the maximum number of clusters
 * for the chip family.
 * @param meid      ME ID as created by NFP3200_MEID
 */
#define NFP3200_MEID_IS_VALID(meid) ((((meid) >> 4) < 5) && \
                                     (((meid) >> 4) >= 0))

/** Extract cluster number from ME ID.
 * @param meid   ME ID as created by NFP3200_MEID
 */
#define NFP3200_MEID_CLUSTER_of(meid) (((meid) >> 4) & 0x7)

/** Extract microengine number (0 based) from ME ID.
 * @param meid   ME ID as created by NFP3200_MEID
 */
#define NFP3200_MEID_MENUM_of(meid) ((meid) & 0x7)

/** Extract microengine group number (0 based) from ME ID.
 *  The group is two code-sharing microengines, so group
 *  0 refers to MEs 0,1, group 1 refers to MEs 2,3 etc.
 * @param meid   ME ID as created by NFP3200_MEID
 */
#define NFP3200_MEID_MEGRP_of(meid)  (NFP3200_MEID_MENUM_of(meid) >> 1)

/** Make microengine ID for NFP-6xxx.
 * @param island_id   Island ID.
 * @param menum       ME number, 0 based, within island.
 *
 * NOTE: menum should really be unsigned - MSC compiler throws error (not warning)
 * if a clause is always true i.e. menum >= 0 if cluster_num is type unsigned int
 * hence the cast of the menum to an int in that particular clause
 */
#define NFP6000_MEID(island_id, menum) \
    (((((int)(island_id) & 0x3F) == (int)(island_id)) && \
    (((int)(menum) >= 0) && ((int)(menum) < 12))) ? \
    (int)(((island_id) << 4) | ((menum) + 4)) : -1)

/** Do a general sanity check on the ME ID.
 * The check is on the highest possible island ID
 * for the chip family and the microengine number must
 * be a master ID.
 * @param meid      ME ID as created by NFP6000_MEID
 */
#define NFP6000_MEID_IS_VALID(meid) ((((meid) >> 4) < 64) && \
                                     (((meid) >> 4) >= 0) && \
                                     (((meid) & 0xF) >= 4))
 ...
 * @param menum         ME number, 0 based, within island.
 */
#define NFP_MEID(chip_family, island, menum)                             \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_MEID(island, menum) :  \
     NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_MEID(island, menum) : -1)

/** Extract island ID from ME ID.
 * @param chip_family    Chip family ID.
 * @param meid          ME ID.
 */
#define NFP_MEID_ISLAND_of(chip_family, meid)                               \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_MEID_ISLAND_of(meid) :    \
     NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_MEID_CLUSTER_of(meid) : -1)

/** Extract cluster ID from ME ID.
 * @param chip_family    Chip family ID.
 * @param meid          ME ID.
 */
#define NFP_MEID_CLUSTER_of(chip_family, meid)                              \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_MEID_ISLAND_of(meid) :    \
     NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_MEID_CLUSTER_of(meid) : -1)
 ...
 * @param meid          ME ID.
 */
#define NFP_MEID_MENUM_of(chip_family, meid)                                \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_MEID_MENUM_of(meid) :     \
     NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_MEID_MENUM_of(meid) : -1)
 ...
 * @param meid   ME ID
 */
#define NFP_MEID_MEGRP_of(chip_family, meid)                                \
    (NFP_FAMILY_IS_NFP6000(chip_family) ? NFP6000_MEID_MEGRP_of(meid) :     \
     NFP_FAMILY_IS_NFP3200(chip_family) ? NFP3200_MEID_MEGRP_of(meid) : -1)

/** Size of char buffer for "2str" functions.
 */
#define NFP_MEID_STR_SZ   (32) /* xxxYY.xxxYY.xxxYY.\0 */
 ...
 *
 * @param s       A string of format "clX.meY" or "clX.meY.anything"
...
/** Extract cluster number from string.
 *
 * Example:
 * char *c;
 * int val = nfp3200_idstr2cluster("cl2.me5", &c);
 * // val == 2, c == "me5"
 * val = nfp3200_idstr2cluster("cl2", &c);
 * // val == 2, c == ""
 *
 * @param s       A string of format "clX.anything" or "clX"
 * @param endptr  If non-NULL, *endptr will point to the trailing string
 *                after the cluster part of the string, which is either
 *                an empty string or the first character after the separating
 *                period.
 * @return        If successful, the cluster number, -1 on error.
 */
NFP_COMMON_API
 ...
 * Example:
 * char *c;
 * int val = nfp3200_idstr2menum("me5.anything", &c);
 * // val == 5, c == "anything"
 * val = nfp3200_idstr2menum("me5", &c);
 * // val == 5, c == ""
 *
 ...
 * Example:
 * char *c;
 * int val = nfp3200_idstr2ctxnum("ctx5.anything", &c);
 * // val == 5, c == "anything"
 * val = nfp3200_idstr2ctxnum("ctx5", &c);
 * // val == 5, c == ""
 *
 ...
 * Example:
 * char *c;
 * int val = nfp3200_idstr2megrp("tg2.anything", &c);
 * // val == 2, c == "anything"
 * val = nfp3200_idstr2megrp("tg5", &c);
 * // val == 2, c == ""
 *
...
/** Create ME ID string of format "clX.meY".
 *
 ...
 * @param s      Pointer to char buffer of size NFP_MEID_STR_SZ.
 *               The resulting string is output here.
 * @param menum  Microengine number within cluster.
 * @return       Pointer to "s" on success, NULL on error.
 */
NFP_COMMON_API
 ...
 */
NFP_COMMON_API
int nfp_idstr2meid(int chip_family, const char *s,
                                 const char **endptr);
 ...
 */
NFP_COMMON_API
int nfp_idstr2island(int chip_family, const char *s,
                                   const char **endptr);
 ...
 */
NFP_COMMON_API
int nfp_idstr2menum(int chip_family, const char *s,
                                  const char **endptr);
 ...
 */
NFP_COMMON_API
int nfp_idstr2ctxnum(int chip_family, const char *s,
                                   const char **endptr);
 ...
 */
NFP_COMMON_API
int nfp_idstr2megrp(int chip_family, const char *s,
                                  const char **endptr);
 ...
 */
NFP_COMMON_API
const char *nfp_island2str(int chip_family, char *s,
                                         int island_id);
 ...
 */
NFP_COMMON_API
const char *nfp_island2altstr(int chip_family, char *s,
                                            int island_id);
 ...
 */
NFP_COMMON_API
int nfp_str2cpp(int chip_family, const char *tid,
                uint32_t *cpp_idp, uint64_t *cpp_addrp);
