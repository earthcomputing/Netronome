...
#define NFP_REGDATA_FLG_IGN_CASE_FULLNAME         (1U << 0)

/* For lookups, ignore the case of altnames, which is upper case */
#define NFP_REGDATA_FLG_IGN_CASE_ALTNAME          (1U << 1)

/* For matches, don't include normal names */
#define NFP_REGDATA_MATCH_FLG_SKIP_FULLNAME       (1U << 2)

/* For matches, don't include altnames */
#define NFP_REGDATA_MATCH_FLG_SKIP_ALTNAME        (1U << 3)

/* For matches, don't include bit fields of a matched register */
#define NFP_REGDATA_MATCH_FLG_SKIP_BITFIELDS      (1U << 4)
...
struct nfp_regdata_node
{
    ...
    /* NFP_REGDATA_FLG_GET_* enables these strings
       and the caller must free with nfp_regdata_node_free */
    char *name;
    char *altname;
    char *description;

    /* This is always the write-offset, which is almost always the same
       as the read offset. */
    uint64_t offset;

    /* The first and last byte offsets occupied by registers
       in a map or its nested maps. In most cases this
       will be 0 to the last byte of the last register in a
       memory-like map and thus the size of such a map can be
       obtained by (reg_byte_last - reg_byte_first + 1).
       Note that 'reg_byte_first' and 'offset' may not be the same
       if the register in the map do not start from 0.
       */
    ...
    /* NFP_REGDATA_IID_NA if not applicable,
    island ID if lookup referred
    to specific island.
    If set, IID may still not be part of offset. Check offset_iid_lsb.
    Will be NFP_REGDATA_IID_LOCAL if the lookup was for a local island (i0). */
    int iid;
    ...
    /* Set to register bit size if type is a regiser or bitfield.
       If type is a map and all the map entries are registers
       with the same bit size, this will also be set to that
       bit size. */
    uint32_t bit_len;
 ...
 */
NFP_COMMON_API
struct nfp_regdata *nfp_regdata_load(
 ...
 */
NFP_COMMON_API
const char *nfp_regdata_comment(
 ...
 */
NFP_COMMON_API
void nfp_regdata_node_free_data(
    struct nfp_regdata_node *node);

/** Get the NFP family of the register data.
 *
 * @param regdata       Handle to register data
 * @return Currently only returns NFP_CHIP_FAMILY_NFP6000
 */
NFP_COMMON_API
int nfp_regdata_family(
    struct nfp_regdata *regdata);
 ...
 */
NFP_COMMON_API
int nfp_regdata_lookup_local(
    struct nfp_regdata *regdata,
    const char *name,
    unsigned int flags,
    struct nfp_regdata_node *node,
    int local_iid, int local_menum);
 ...
 */
NFP_COMMON_API
enum nfp_reg_type nfp_regdata_regid2type(
    struct nfp_regdata *regdata,
 ...
 */
NFP_COMMON_API
ssize_t nfp_regdata_mecsr_list(
    struct nfp_regdata *regdata,
