...
#define NFP_REGDB_TABLE_ID(_island_, _target_, \
    _wr_act_, _wr_tok_, _rd_act_, _rd_tok_, \
    _owner_) \
...
#define NFP_REGDB_ENTRY_WORD_W0_VTP_of(_w0_) ((enum nfp_regdb_valtype) \
    (((_w0_) >>  0) &  0x7))
...
struct nfp_regdb_entry_word {
    /* w0 and offset_lo must be the same for all higher words of a multi-word
     * entry (register larger than 32 bits), except for "last_word" which
     * will only be set for the last 32-bit word.
     */
    uint32_t w0;
    uint32_t offset_lo;
    uint32_t value;
 ...
 */
NFP_COMMON_API
struct nfp_regdb *nfp_regdb_from_buf(
 ...
 */
NFP_COMMON_API
size_t nfp_regdb_table_entries(
    struct nfp_regdb *db,
    uint32_t table_id,
 ...
 */
NFP_COMMON_API
int nfp_regdb_word_conflicts(
    const struct nfp_regdb_entry_word *existing,
 ...
 */
NFP_COMMON_API
int nfp_regdb_val_type_cmp(
    enum nfp_regdb_valtype a,
 ...
 */
NFP_COMMON_API
int nfp_regdb_entry_regcmp(
    const struct nfp_regdb_entry *a,
    const struct nfp_regdb_entry *b);
