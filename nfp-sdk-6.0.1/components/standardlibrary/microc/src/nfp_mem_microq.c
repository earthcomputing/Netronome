...
#define _MEM_DECLARE_AND_SET_HI_AND_LOW_ADDR(addr)                          \
        ...
        hi_addr = (unsigned int)((unsigned long long)addr>>8 & 0xFFFFFFFF); \
        low_addr = (unsigned int)((unsigned long long)addr & 0xFF);
...
)
{
    _INTRINSIC_BEGIN;
    {
        _MEM_MICROQ_ALIGN_16_CHECK((unsigned int)microq);
