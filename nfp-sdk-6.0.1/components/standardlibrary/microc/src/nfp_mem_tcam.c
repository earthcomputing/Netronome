...
{
    mem_write_atomic_imm_ptr32(0, (void __addr32 __mem *) tcam, sizeof(*tcam) / sizeof(unsigned int));
}
...
{
    mem_write_atomic_imm_ptr40(0, tcam, sizeof(*tcam) / sizeof(unsigned int));
}
#endif
...
{
    __addr32 __mem unsigned int *fp = (__addr32 __mem unsigned int *)tcam;
    mem_write_atomic_imm_ptr32(0, (void __addr32 __mem *)tcam, 8);
...
{
    __addr40 __mem unsigned int *fp = (__addr40 __mem unsigned int *)tcam;
    mem_write_atomic_imm_ptr40(0, tcam, 8);
...
{
    __addr32 __mem unsigned int *fp = (__addr32 __mem unsigned int *)tcam;
    mem_write_atomic_imm_ptr32(0, (void __addr32 __mem *)tcam, sizeof(*tcam) / sizeof(unsigned int) / 2);
    mem_write_atomic_imm_ptr32(0, fp + sizeof(*tcam) / sizeof(unsigned int) / 2, sizeof(*tcam) / sizeof(unsigned int) / 2);
}
...
{
    __addr40 __mem unsigned int *fp = (__addr40 __mem unsigned int *)tcam;
    mem_write_atomic_imm_ptr40(0, tcam, sizeof(*tcam) / sizeof(unsigned int) / 2);
    mem_write_atomic_imm_ptr40(0, fp + sizeof(*tcam) / sizeof(unsigned int) / 2, sizeof(*tcam) / sizeof(unsigned int) / 2);
}
#endif
...
{                                                                   \
    ...
    {                                                               \
        ...
        generic_ind_t ind;                                          \
        __addr32 __mem unsigned int *ptr = (__addr32 __mem unsigned int *)tcam;                   \
...
{                                                                   \
    ...
    {                                                               \
        __xwrite unsigned int v;                                    \
        generic_ind_t ind;                                          \
        __addr40 __mem unsigned int *ptr = (__addr40 __mem unsigned int *)tcam; \
        v = (value << 24) >> ((index & 3) << 3);                    \
        ptr += (index >> 2);                                        \
        _INTRINSIC_OVERRIDE_BYTEMASK(ind, 8 >> (index & 3));        \
...
{                                                                   \
    ...
    {                                                               \
        ...
        mem_write_atomic_ind_ptr32(&v,(__addr32 __mem void*)ptr,2,ind,sync,sig_ptr);     \
    }                                                               \
}
...
{                                                                   \
    ...
    {                                                               \
        ...
        ptr += (word_index >> 2);                                   \
        _INTRINSIC_OVERRIDE_BYTEMASK(ind, 0x88 >> (entry_index & 3));               \
        mem_write_atomic_ind_ptr40(&v,(__addr40 __mem void*)ptr,2,ind,sync,sig_ptr); \
    }                                                               \
}
...
{                                                                   \
    ...
    {                                                               \
        ...
        __addr32 __mem unsigned int *ptr = (__addr32 __mem unsigned int*)tcam + index;            \
...
{                                                                   \
    ...
    {                                                               \
        __xwrite unsigned int v = value;                            \
        __addr40 __mem unsigned int *ptr = (__addr40 __mem unsigned int*)tcam + index; \
...
{                                                                   \
    ...
    CT_ASSERT(__is_ct_const(sync));                                 \
    {                                                               \
        unsigned int word_index = (entry_index << 1);               \
        __xwrite unsigned int v = value;                            \
        __addr32 __mem unsigned int *ptr = (__addr32 __mem unsigned int*)tcam + word_index;       \
        mem_write_atomic_ptr32(&v,(__addr32 __mem void*)ptr,1,sync,sig_ptr);             \
    }                                                               \
}
...
{                                                                   \
    ...
    {                                                               \
        unsigned int word_index = (entry_index << 1);               \
        __xwrite unsigned int v = value;                            \
        __addr40 __mem unsigned int *ptr = (__addr40 __mem unsigned int*)tcam + word_index; \
        mem_write_atomic_ptr40(&v,(__addr40 __mem void*)ptr,1,sync,sig_ptr); \
    }                                                               \
}
...
{                                                                   \
    ...
    CT_ASSERT(__is_ct_const(sync));                                 \
    {                                                               \
        unsigned int word_index = (entry_index << 1) + 1;           \
        __xwrite unsigned int v = mask;                \
        __addr32 __mem unsigned int *ptr = (__addr32 __mem unsigned int*)tcam + word_index;       \
        mem_write_atomic_ptr32(&v,(__addr32 __mem void*)ptr,1,sync,sig_ptr);             \
    }                                                               \
}
...
{                                                                   \
    ...
    {                                                               \
        __xwrite unsigned int v[2];                                 \
        __addr32 __mem unsigned int *ptr;                           \
        unsigned int word_index = (entry_index << 1);               \
        v[0] = value;                                               \
        v[1] = mask;                                                \
        ptr = (__addr32 __mem unsigned int*)tcam + word_index; \
        mem_write_atomic_ptr32(&v,(__addr32 __mem void*)ptr,2,sync,sig_ptr);             \
    }                                                               \
}
...
{                                                                   \
    ...
    {                                                               \
        __xwrite unsigned int v[2];                                 \
        __addr40 __mem unsigned int *ptr;                           \
        unsigned int word_index = (entry_index << 1);               \
        v[0] = value;                                               \
        v[1] = mask;                                                \
        ptr = (__addr40 __mem unsigned int*)tcam + word_index; \
        mem_write_atomic_ptr40(&v,(__addr40 __mem void*)ptr,2,sync,sig_ptr);             \
    }                                                               \
}
...
__intrinsic                                                                     \
unsigned int mem_tcam_entry_value_to_word_index8(                               \
...
__intrinsic                                                                     \
unsigned int mem_tcam_entry_mask_to_word_index8(                                \
    unsigned int entry_idx)                                                     \
{                                                                               \
...
__intrinsic                                                                     \
unsigned int mem_tcam_entry_value_to_word_index16(                              \
    unsigned int entry_idx)                                                     \
{                                                                               \
...
__intrinsic                                                                     \
unsigned int mem_tcam_entry_mask_to_word_index16(                               \
    unsigned int entry_idx)                                                     \
{                                                                               \
...
__intrinsic                                                                     \
unsigned int mem_tcam_entry_value_to_word_index32(                              \
    unsigned int entry_idx)                                                     \
{                                                                               \
    return (entry_idx << 1);                                                    \
}
...
__intrinsic                                                                     \
unsigned int mem_tcam_entry_mask_to_word_index32(                               \
    unsigned int entry_idx)                                                     \
{                                                                               \
